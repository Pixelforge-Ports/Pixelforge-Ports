...
for record in records:
    meta=record['meta'];game=record['id'];title=meta['title'];readme=record['readme']
    for folder in ['assets/games','guides']:(OUT/folder).mkdir(parents=True,exist_ok=True)
    (OUT/'assets/games'/(game+'.png')).write_bytes(record['screenshot'])
    digest=record['sha256']
    data={k:record[k] for k in ['id','zip','size','sha256','repository','ref','download_url','release_url','version','prerelease']}
    requirements=game_requirements(readme)
    data.update(title=title,description=meta['desc'],genres=meta['genres'],windows_version=requirements['windows_version'],game_version=requirements['game_version'],game_platform=requirements['platform'])
    catalog.append(data)
    size=f"{record['size']/1024:.0f} KB" if record['size']<1048576 else f"{record['size']/1048576:.1f} MB"
    archive_name=record['zip']
    status=('Testing release ' if record['prerelease'] else 'Release ')+str(record['version']) if record['version'] else 'No published download yet'
    download=(f'<a class="button primary" href="{esc(record["download_url"])}">↓ Download from GitHub <small>{size}</small></a>'
              if record['download_url'] else '<p class="notice">No published port ZIP yet. Check the repository for progress.</p>')
    download+=f'<p class="small">{esc(status)}</p><a class="text-link" href="{esc(record["repository"])}">Source &amp; updates on GitHub ↗</a>'
    checksum=(f'<details><summary>Package checksum (GitHub)</summary><code class="checksum">{digest}</code></details>' if digest else '')
...
for file in (OUT/'guides').glob('*.html'):
    if file.stem not in known:file.unlink()
for file in (OUT/'assets/games').glob('*.png'):
    if file.stem not in known:file.unlink()
for file in (OUT/'downloads').glob('*'):
    if file.is_file() and (file.suffix.lower()=='.zip' or file.name=='SHA256SUMS.txt'):file.unlink()
...
print(f'Catalog ready: {len(catalog)} GitHub ports with release links and individual guides.')
